...
spec:
  ...
  interval: 1h
  values:
    fullnameOverride: forgejo-runner-controller
    forgejo:
      url: https://forgejo.example.com
      apiTokenSecret:
        name: forgejo-runner-controller-secret   # Pre-existing or ExternalSecret-managed
        key: token
      scope: admin
```

For a complete Flux CD example including Kustomize layout and External Secrets integration, see [`examples/kubernetes/flux-cd/`](../../examples/kubernetes/flux-cd/).
